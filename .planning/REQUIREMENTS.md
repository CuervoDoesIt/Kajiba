...
- [x] **PRIV-07**: User can request deletion of a contributed record via `kajiba delete <record_id>`
- [x] **PRIV-08**: Deletion requests are tracked in a deletion index file in the dataset repository

### Data Quality

- [x] **QUAL-01**: Quality tier (gold/silver/bronze/review_needed) and composite score are stored in the outbox record at submit time
...
- [x] **QUAL-03**: User can rate a staged record's quality via `kajiba rate` with a numeric score and optional tags
- [x] **QUAL-04**: User can report pain points on a staged record via `kajiba report` with category, description, and severity
- [x] **QUAL-05**: User annotations (ratings, pain points) are included in the exported record alongside auto-scores
...
- [x] **PUB-05**: User can publish scrubbed records to the dataset repository via `kajiba publish`
...
- [x] **CONS-03**: User can browse the dataset catalog via `kajiba browse` with filters for model, tier, and hardware
- [x] **CONS-04**: User can download a filtered subset of the dataset via `kajiba download` with model/tier/hardware filters

### Contribution Modes

- [x] **CONT-01**: User can contribute in ad-hoc mode — review and approve each record before submission
- [x] **CONT-02**: User can contribute in continuous mode — records meeting configured quality threshold are auto-submitted
